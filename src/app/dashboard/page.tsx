...
const DashboardCharts = dynamic(
    ...
    { ssr: false, loading: () => <div className="h-[220px] flex items-center justify-center text-sm text-neutral-300">Cargando gráficas...</div> }
);

const CronometrajeSection = dynamic(
    () => import('@/components/dashboard/CronometrajeSection'),
    { ssr: false, loading: () => <div className="h-40 flex items-center justify-center text-sm text-neutral-400">Cargando gráficas...</div> }
);
                        ...
                        <div className="flex items-center gap-4">
                            {([
                                ...
                                <label key={key} className="flex items-center gap-1.5 cursor-pointer select-none">
                                    <input type="checkbox" checked={visibleLines[key]} onChange={() => toggleLine(key)} className="sr-only" />
                                    <span className="w-3 h-3 rounded-sm flex-shrink-0 border-2 transition-colors" style={{ backgroundColor: visibleLines[key] ? color : 'transparent', borderColor: color }} />
